/******************************************************************************
 * $Header: //depot/HTE/QDART/QMSL6.1/QMSL_Projects/QMSL_NVTool/QMSL_NVTool.cpp#29 $
 * $DateTime: 2017/09/27 20:03:30 $
 *
 *
 ******************************************************************************
 *
...
{
   DbgPrint("Subscription ID:%u, %s, Function:%d, Event:%d, <NV_Process_Progress: %d%s>\n", iSubscriptionId, sRFNVid_EFSPath, iNVToolFuncEnum, iEvent, iProgress, "%");
}
...
// Extract source format for <NvItem> vs <NvItemData>
int ExtractSrcFormat(string argument);
...
bool Qpst2Qcn();  //read NV values from mobile using QPST and write to QCN\n");
...
bool Src2NVItemData(); // read variant nv from a source file and write to another source file in NVDataType format
bool Src2Src(); 
bool MergeQcn();
...
// File list to store the qcn file to be written for SRC2NVITEMDATA option
vector<string> gSrcOutFileList;
...
// Use NV Item Data Format when generateing source, 0 = NvItem 1=NVItemData
int gNVItemDataFormat = 0;
// Use strict or loose policy to load NVs data from source file. 
// strict policy.  Load NVs failed even one item is failed
// loose policy.  Load NVs will not fail even the data in a NV item is invalid
const int NVTOOL_LOAD_NVs_FROM_SRC_STRICT = 1;
const int NVTOOL_LOAD_NVs_FROM_SRC_LOOSE = 0;
int gLoadNVSourcePolicy = NVTOOL_LOAD_NVs_FROM_SRC_STRICT;
...
int _tmain(int argc, _TCHAR* argv[])
{
   ...
   if(CheckArguments() == true)
   {
      ...
      {
         DbgPrint("Dispatch Command failed\n");
   ...
   {
      DbgPrint("Failed to launched any action. Check argument list!\n");
     return rc = NVTOOL_ARGUMENT_INVALID;
   }

   DbgPrint("Total Execution Time: %d ms\n", ::GetTickCount() - _timeBegin); 
   return rc;
}
...
void ParseArguments()
{
   ...
   if((_position = action.find("-src2qcn")) == 0)
      gCmdAction = SRC2QCN;
   else if((_position = action.find("-src2src")) == 0)
      gCmdAction = SRC2SRC;
   else if((_position = action.find("-src2ph")) == 0)
      gCmdAction = SRC2PH;
   else if((_position = action.find("-qpst2qcn")) == 0)
   {
      gCmdAction = QPST2QCN;
      bQPST = 1;
   }
    ...
    else if((_position = action.find("-src2nvitemdata")) == 0)
      gCmdAction = SRC2NVITEMDATA;
    else if((_position = action.find("-mergeqcn")) == 0)
       gCmdAction = MERGEQCN;
   ...
   for(_i = 2; _i < gArgumentList.size(); _i++)
   {
      ...
      else if((_position = argument.find("-outsrc")) == 0)
      {
         string fileName = ExtractFileName(argument, 8);
         gSrcOutFileList.push_back(fileName);
      }
      ...
      else if((_position = argument.find("-format")) == 0)
      {
          gNVItemDataFormat = ExtractSrcFormat(argument);
      }
	  else if((_position = argument.find("-loose")) == 0)
      {
          gLoadNVSourcePolicy = NVTOOL_LOAD_NVs_FROM_SRC_LOOSE;
      }
   }
}
...
bool CheckArguments()
{
   ...
   case SRC2QCN:
      ...
      break;
   case SRC2SRC:
      if((gDefFileList.size() >= 1)&&(gSrcFileList.size() >= 1) && (gSrcOutFileList.size() >= 1))
         _bResult = true;
      break;
   case SRC2PH:
   ...
   case QCN2SRC:
      if((gQcnFileList.size() == 1) && (gSrcFileList.size() == 1) && (gDefFileList.size() >= 0))
         _bResult = true;
      break;
   case BDF2SRC: // intention fall through
   case SRC2BDF:
      if((gBDFileList.size() == 1) && (gSrcFileList.size() == 1) && (gDefFileList.size() >= 0))
         _bResult = true;
      break;
   case BDF2PH: // intention fall through
   case PH2BDF:
   ...
   case QPST2QCN:
      _bResult = true;
      if(gQcnFileList.size() != 1)
      {
         DbgPrint("Missign argument: -qcn\n");   
         _bResult = false;
      }
      break;
   }
...
bool DispatchCommand()
{
   ...
   if(gCmdAction == SRC2QCN)
      ...
      bResult = Src2Ph();
   else if(gCmdAction == SRC2SRC)
       bResult = Src2Src();
   else if(gCmdAction == PH2QCN)
      bResult = Ph2Qcn();
   else if(gCmdAction == QPST2QCN)
      bResult = Qpst2Qcn();
   ...
   else if(gCmdAction == SRC2NVITEMDATA)
       bResult = Src2NVItemData();
   else if(gCmdAction == MERGEQCN)
      bResult = MergeQcn();
...
void PrintUsage()
{
   ...
   printf("\t-src2src   read NV values from NV source xml and write to NV source, use -format to specify output XML type\n");   
   ...
   printf("\t-qpst2qcn   read NV values from mobile using QPST SW download, which is probably faster than -ph2qcn option, and write to QCN\n");
   ...
   printf("\t-src2nvitemdata   read NV values from a source file and write into a source file in the standard <NvItemData> format\n");
   printf("\t-mergeqcn  read NV values from xml src file(s) and merge the results into a qcn file\n");
   printf("\t-loose  load NVs from source with loose policy.  Benign error in a NV item data will not cause NVTool stop loading data.  By default, the tool uses a STRICT policy\n");
   ...
   printf("\t -outsrc=<filename> Output src file for -src2nvitemdata option \n");
   printf("\t -speed=<Sleep Counter> The argument for QLIB_Set_QPHONEMS_Userdefined_Sleep_Counter. For example, -speed=1\n");
   printf("\t -nv=the comma separated list of RFNV used by -varaintnv2src action\n");
   printf("\t -datatype=the comma separated list of Datatype ID used by -varaintnv2src action\n");
   printf("\t -format=<format> 1=<NvItemData> 0=<NvItem>  Format use to generating source file\n");
...
{
   int _srcFormat = 0;
   string comPort = argument.erase(0, 8);
   _srcFormat = atoi(comPort.c_str());
   return _srcFormat;
}
...
bool LoadReadFilter()
{
   ...
   for(_i = 0; _i < ::gReadFilterList.size(); _i++)
   {
      ...
      DbgPrint("QLIB_NV_LoadReadFilter:%s, _iResultCode= %d\n", gReadFilterList[_i].c_str(), _iResultCode);
...
bool LoadWriteFilter()
{
   ...
   for(_i = 0; _i < ::gWriteFilterList.size(); _i++)
   {
      ...
      DbgPrint("QLIB_NV_LoadWriteFilter:%s, _iResultCode=%d\n", gWriteFilterList[_i].c_str(), _iResultCode);   
...
bool LoadNVDefinitionFile()
{
   ...
   {   
      ...
      DbgPrint("QLIB_NV_LoadNVDataTypeDictinary Def:%s, _iNumNVDef=%d, _iResultCode=%d\n", gDefFileList[_i].c_str(), _iNumNVDef, _iResultCode);

      ::QLIB_NV_LoadNVDefinitionFile(gQMSL, gDefFileList[_i].c_str(), &_iNumNVDef, &_iResultCode);
      DbgPrint("QLIB_NV_LoadNVDefinitionFile Def:%s, _iNumNVDef=%d, _iResultCode=%d\n", gDefFileList[_i].c_str(), _iNumNVDef, _iResultCode);

      if((_iResultCode == MSXML_FAILED_TO_LOAD_DEF) || (_iResultCode == MSXML_FAILED_TO_INSTANTIATE))
      {
        DbgPrint("LoadNVDefinitionFile fails %d, most likely your PC needs to install msxml\n", _iResultCode);
         bResult = false;
      }
   }
   return bResult;
...
bool LoadSrcFiles()
{
   ...
   for(_i = 0; _i < ::gSrcFileList.size(); _i++)
   {
      ...
      DbgPrint("QLIB_NV_LoadNVsFromSource source:%s, _iNumNVValues=%d, _iResultCode=%d\n", gSrcFileList[_i].c_str(), _iNumNVValues, _iResultCode);
      
	  // Strict policy. more failure condition will stop the tool from proceeding
	  if(gLoadNVSourcePolicy == NVTOOL_LOAD_NVs_FROM_SRC_STRICT)
	  {
	  ...
	  // Loose policy
	  else if(gLoadNVSourcePolicy == NVTOOL_LOAD_NVs_FROM_SRC_LOOSE)
	  {
...
bool LoadQCNFiles()
{
   ...
   for(_i =0; _i < ::gQcnFileList.size(); _i++)
   {
      ...
      DbgPrint("QLIB_NV_LoadNVsFromQCN Qcn:%s, _iNumNVValues=%d, _iResultCode=%d\n", gQcnFileList[_i].c_str(), _iNumNVValues, _iResultCode);
...
bool LoadBDFFromMobile()
{
   ...
   DbgPrint("LoadBDFFromMobile QLIB_FTM_WLAN_TLV_Create -- _OP_BDF_RW=%d\n", OP_BDF_RW);

    ::QLIB_FTM_WLAN_TLV_AddParam(gQMSL, "WLANBDF_actions", "1");    // _WLAN_BDFaction_READ
   DbgPrint("LoadBDFFromMobile QLIB_FTM_WLAN_TLV_AddParam -- WLANBDF_actions=_WLAN_BDF_READ\n");

    ::QLIB_FTM_WLAN_TLV_AddParam(gQMSL, "BDFNamePathPhone", (char*)gBDFileList[0].c_str());
   DbgPrint("LoadBDFFromMobile QLIB_FTM_WLAN_TLV_AddParam -- BDFNamePathPhone=%s\n", (char*)gBDFileList[0].c_str());

    ::QLIB_FTM_WLAN_TLV_AddParam(gQMSL, "BDFNamePathQDART", (char*)gSrcFileList[0].c_str());
   DbgPrint("LoadBDFFromMobile QLIB_FTM_WLAN_TLV_AddParam -- BDFNamePathQDART=%s\n", (char*)gSrcFileList[0].c_str());

    ::QLIB_FTM_WLAN_TLV_Complete(gQMSL);
   DbgPrint("LoadBDFFromMobile QLIB_FTM_WLAN_TLV_Complete\n");
...
bool LoadNVsFromBoardDataFile()
{
   ...
   DbgPrint("QLIB_NV_LoadNVsFromBDF _iNumNVValues=%d _iResultCode=%d\n", _iNumNVValues, _iResultCode);
...
bool SaveNVsToBoardDataFile()
{
   ...
   DbgPrint("QLIB_NV_WriteNVsToBDF BDF=%s _iResultCode=%d\n", gBDFileList[0].c_str(), _iResultCode);
...
bool LoadNVsFromMobile()   // Load NVs from mobile
{
   ...
   DbgPrint("QLIB_GetPhoneOperatingMode : Phone Mode Return =%d\n", _iPhoneMode);

   if(_iPhoneMode != SYS_OPRT_MODE_FTM)
   {
      _bSetModeFTM = ::QLIB_DIAG_CONTROL_F(gQMSL, MODE_FTM_F);
      DbgPrint("QLIB_DIAG_CONTROL(h: MODE_FTM_F) status %d\n", _bSetModeFTM);
      Sleep(500);
   }

   ::QLIB_NV_LoadNVsFromMobile(gQMSL, &_iNumNVValues, &_iResultCode);
   DbgPrint("QLIB_NV_LoadNVsFromMobile _iNumNVValues=%d _iResultCode=%d\n", _iNumNVValues, _iResultCode);
...
bool SaveNVsToMobile()      // Save NVs to mobile
{
   ...
   DbgPrint("QLIB_NV_WriteNVsToMobile _iResultCode=%d\n",  _iResultCode);
   ...
   if(bSyncEFS)
   {
      ...
      DbgPrint("QLIB_EFS2_SyncWithWait: _iExecutionStatus =%d\n", _iExecutionStatus);
   }   
...
bool SaveNVsToQCN()         // Save NVs to QCN
{
   ...
   DbgPrint("QLIB_NV_WriteNVsToQCN Qcn:%s, _iResultCode=%d\n", gQcnFileList[0].c_str(), _iResultCode);
   if(_iResultCode == FAILED_TO_OPEN_QCN)
   {
      bResult = false;
   }
...
bool SaveNVsToSrc(bool dataFromBin)         // Save NVs to Source
{
   ...
   // Use NV Item Format
   if(gNVItemDataFormat == 1)
    ::QLIB_NV_WriteNVsToSourceSchema(gQMSL, 1);
   
   ::QLIB_NV_WriteNVsToSource(gQMSL, gSrcFileList[0].c_str(), &_iResultCode, one);   
   DbgPrint("QLIB_NV_WriteNVsToSource Src:%s, _iResultCode=%d\n", gSrcFileList[0].c_str(), _iResultCode, dataFromBin);
   
   if((_iResultCode == MSXML_FAILED_TO_SAVE_SRC) || 
       (_iResultCode == MSXML_FAILED_TO_INSTANTIATE) || 
       (_iResultCode == FAILED_TO_OPEN_SRC) )
   {
     DbgPrint("QLIB_NV_WriteNVsToSource Src failed");
      bResult = false;
   }
   return bResult;
...
bool SaveNVsToTAR()         // Save NVs to QCN
{
   ...
   DbgPrint("QLIB_NV_WriteNVsToTAR Qcn:%s, _iResultCode=%d\n", gTarFileList[0].c_str(), _iResultCode);
...
bool Src2Qcn()
{
   ...
   int _iNumNVValues = 0;
   int _iResultCode = 0;
   DbgPrint("QMSL_NVTool:NV Source to QCN\n");
...
bool Src2Src()
{
   ...
   int _iNumNVValues = 0;
   int _iResultCode = 0;
   DbgPrint("QMSL_NVTool: NV xml to NV xml in %s format\n", ((0 == gNVItemDataFormat)?"NvItem":"NvItemData"));

   if(LoadNVDefinitionFile() &&
      LoadSrcFiles())
   {
      ::QLIB_NV_WriteNVsToSourceSchema(gQMSL, gNVItemDataFormat);
      ::QLIB_NV_WriteNVsToSource(gQMSL, gSrcOutFileList[0].c_str(), &_iResultCode, 0);   
      DbgPrint("QLIB_NV_WriteNVsToSource Src:%s, _iResultCode=%d\n", gSrcOutFileList[0].c_str(), _iResultCode, false);
      bResult = true;
      if((_iResultCode == MSXML_FAILED_TO_SAVE_SRC) || 
         (_iResultCode == MSXML_FAILED_TO_INSTANTIATE) || 
         (_iResultCode == FAILED_TO_OPEN_SRC) )
...
bool Src2Ph()
{
   ...
   int _iNumNVValues = 0;
   int _iResultCode = 0;
   DbgPrint("QMSL_NVTool:NV Source to Mobile\n");
...
bool Ph2Qcn()
{
   ...
   int _iNumNVValues = 0;
   int _iResultCode = 0;
   DbgPrint("QMSL_NVTool:Mobile to QCN\n");
...
bool Qpst2Qcn()
{
   ...
   int _iNumNVValues = 0;
   int _iResultCode = 0;
   DbgPrint("QMSL_NVTool:Mobile via QPST SW download to QCN\n");

   if(QLIB_DownloadQcnFile(gQMSL, (char*)(gQcnFileList[0].c_str()), "0000000"))      
   {
      bResult = true;
   }
   return bResult;   
...
bool Ph2Src()
{
   ...
   DbgPrint("QMSL_NVTool:Mobile to NV source\n");
   if(   LoadNVDefinitionFile() &&
      LoadReadFilter() &&
      LoadNVsFromMobile() &&
      LoadWriteFilter() &&
...
bool Src2BDF()
{
   ...
   int _iNumNVValues = 0;
   int _iResultCode = 0;
   DbgPrint("QMSL_NVTool: NV xml to board data file\n");
...
bool BDF2Src()
{
   ...
   DbgPrint("QMSL_NVTool:board data file to NV source\n");
   if(   LoadNVDefinitionFile() &&
//      LoadReadFilter() &&
...
bool Ph2BDF()
{
   ...
   DbgPrint("QMSL_NVTool:Mobile to BDF\n");
...
{
   bool bResult = false;
   DbgPrint("QMSL_NVTool:BDF to Mobile\n");
   if(   SaveBDFToMobile() )
   {
      bResult = true;
   }
...
bool Qcn2Ph()
{
   ...
   int _iNumNVValues = 0;
   int _iResultCode = 0;
   DbgPrint("QMSL_NVTool:QCN to Mobile\n");
...
bool Qcn2Src()
{
   ...
   DbgPrint("QMSL_NVTool:QCN to NV Source\n");
...
bool CreateCompNVFilter()
{
   ...
   DbgPrint("QLIB_NV_CreateComplementNVFilter: Filter=%s, _iMaxNVId=%d _iResultCode=%d\n", gFilterFileList[0].c_str(), _iMaxNVId, _iResultCode);
   return true;
}
...
bool Src2Tar()
{
   ...
   int _iNumNVValues = 0;
   int _iResultCode = 0;
   DbgPrint("QMSL_NVTool: NV Source to TAR and GZ\n");
