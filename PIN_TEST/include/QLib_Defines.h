/******************************************************************************
 * $Header: //depot/HTE/QDART/QMSL6.1/QMSL_inc/QLib_Defines.h#381 $
 * $DateTime: 2016/08/03 18:46:50 $
 *
...
#define MAX_GSM_CAL_RGI             32
#define MAX_GSM_DPD_TBL_SIZE        128
...
typedef enum
{
   ...
   _LTE_RX_CHAIN_SELECTION_COMMAND = 0x4009,
...
typedef enum
{
   _LTE_RX_CHAIN_SELECTION_TYPE_NO_ACTION = 0,
   _LTE_RX_CHAIN_SELECTION_TYPE_ENABLE_RX0_ONLY = 1,
   _LTE_RX_CHAIN_SELECTION_TYPE_ENABLE_RX1_ONLY = 2,
   _LTE_RX_CHAIN_SELECTION_TYPE_ENABLE_RX0_AND_RX1 = 3,
   _LTE_RX_CHAIN_SELECTION_TYPE_ENABLE_RX2_ONLY = 4,
   _LTE_RX_CHAIN_SELECTION_TYPE_ENABLE_RX3_ONLY = 8,
   _LTE_RX_CHAIN_SELECTION_TYPE_ENABLE_RX2_AND_RX3 = 0xC,
   _LTE_RX_CHAIN_SELECTION_TYPE_ENABLE_RX0_RX1_RX2_RX3 = 0xF,
   _LTE_RX_CHAIN_SELECTION_TYPE_END_OF_ENUM = 0x100
} lte_rx_chain_selection_enum_type;
...
typedef enum
{
   ...
   _FTM_SET_XPT_TXAGC_OVERRIDE_V2 = 661,
   ...
   _FTM_QLNK_TEST_SETUP = 1007,

   _FTM_QLNK_TEST_END = 1008,

   _FTM_QLNK_ENUM_CYCLE = 1009,

   _FTM_QLNK_STATE_TRANSIT = 1010,

   _FTM_QLNK_PEAK_TPUT = 1011,

   _FTM_QLNK_ENUM_COLD_START = 1012,
...
typedef enum
{
   ...
   _FTM_RF_TEST_CMD_WAIT_TRIGGER = 10,   /*!< 10 : Wait Trigger: Blocking call while waiting for triggering event */
...
typedef enum
{
   FTM_GSM_CAL_MOD_TYPE_8PSK = 0,

   FTM_GSM_CAL_MOD_TYPE_GMSK = 1,
...
typedef enum
{
   ...
   _FTM_SECURITY_ID_SENSOR_READ_IMAGE_INFO         = 21,       //!<' Return information about image stored in image buffer
...
typedef enum
{
   ...
   _FTM_LTE_NS_RX_SELECT_CHAIN_REQ          = 541   //'<' This command select Rx chain
...
// Define channel frequency range (LOW/MID/HIGH)
#define CHANNEL_RANGE_ENUM_START     0
// Define LTE bandwidth 
#define BH_LTE_CH_BW_ENUM_START    0
...
/**
The enumeration is used by band helper functions (Define channel frequency range (LOW/MID/HIGH))
*/
typedef enum
{
   LOW_CHANNEL = CHANNEL_RANGE_ENUM_START,
   MID_CHANNEL,
   HIGH_CHANNEL,
   CHANNEL_RANGE_END,
   CHANNEL_RANGE_UNKNOWN,
} CHANNEL_RANGE_ENUM;

/**
The enumeration is used by band helper functions (Define LTE bandwidth)
*/
typedef enum
{
   BH_LTE_CH_BW_1_4_MHZ = BH_LTE_CH_BW_ENUM_START,
   BH_LTE_CH_BW_03_MHZ,
   BH_LTE_CH_BW_05_MHZ,
   BH_LTE_CH_BW_10_MHZ,
   BH_LTE_CH_BW_15_MHZ,
...
typedef struct
{
  uint8 num_ca_rx_paths;  //Will be 20 currently {5 CCs * 4 Paths}
  ftm_log_lte_ca_enabled_rx_agc_data_path_type data_path[20];    /* Rx AGC Log packet per path. */
...
typedef enum
{
   RESULT_GSM_TX_CALIBRATION_DATA_RFM_PATH_INFO = 0,
   RESULT_GSM_TX_CALIBRATION_DATA_NVID = 1,
   RESULT_GSM_TX_CALIBRATION_DATA_DA_TYPE = 2,
   RESULT_GSM_TX_CALIBRATION_DATA_DPD_TYPE = 3,
   RESULT_GSM_TX_CALIBRATION_DATA_THERM_TYPE = 4,
   RESULT_GSM_TX_CALIBRATION_DATA_MAX = 0xFFFF
} GSM_TX_RESULT_DATA_TYPE;
...
typedef enum
{
   ...
   XPT_TX_OVERRIDE_MEAS_SEG_LENGTH = 10,
   /* Disabling Memory Kernel */
   XPT_MEMORY_KERNEL_CALIBRATION = 11,
   XPT_PARMS_INVALID
...
typedef struct
{
   ...
   uint16 agcSettlingTime_us; //WTR ADC Settling time

   uint16 accumulationTime_us; //Samples Accumulation Time

   uint8 measType;

   int16 preCalRSBMaxLim_dB;
...
typedef struct
{
   ...
   uint16 tx_override_seg_len;

   /* Parameter to Enable/Disable Memory Kernel DPD */
   byte enableMemoryKernel;
...
typedef struct
{
   ...
   int16 exp_pwr_dBm10[TXLIN_NUM_SEGMENTS];
...
typedef struct
{
  ...
  uint16 num_rgi_entries;

  uint16 rgi[MAX_GSM_CAL_RGI];

  int16 pwrdBm100[MAX_GSM_CAL_RGI];

}gsm_tx_cal_result_per_pa_mod_type;

typedef struct
{
  uint8 pa_state;

  int16 amam_max_dBm100;

  uint16 dpd_predist_rgi;

}gsm_tx_dpd_cal_common_result_type;

typedef struct
{
  uint16 amam_table_size;

  uint16 ampm_table_size;

  uint32 channel;
  
  uint16 amam[MAX_GSM_DPD_TBL_SIZE];

  uint16 ampm[MAX_GSM_DPD_TBL_SIZE];

}gsm_tx_dpd_cal_result_per_channel_type;

typedef struct
{
  uint8 num_pa_states;

  uint8 num_mod_indexes;

  uint32 channel;

  gsm_tx_cal_result_per_pa_mod_type gsm_tx_cal_result_per_pa_mod_data_ptr[MAX_PA_STATES][MAX_GSM_MODULATION_TYPES];/* [paStateIndx][ModulationIndex] */

}gsm_tx_da_cal_result_per_channel_type;

typedef struct
{
  uint8 num_channels;

  gsm_tx_da_cal_result_per_channel_type gsm_tx_da_cal_result_channel_ptr[8];
}gsm_tx_da_cal_result_type;

typedef struct
{
  gsm_tx_dpd_cal_common_result_type dpd_common_result_type;

  gsm_tx_dpd_cal_result_per_channel_type dpd_cal_result_data_ptr[8];

}gsm_tx_dpd_cal_result_type;
...
typedef struct
{
  ...
  uint8 cal_type_mask; /* BIT 0 = DA CAL; BIT 1 = DPD CAL */

  uint8 cal_status;

  uint16 band;

  uint64 result_error_mask;

  rfm_path_type rfm_path;

  gsm_tx_da_cal_result_type gsm_tx_da_cal_result_data;

  gsm_tx_dpd_cal_result_type gsm_tx_dpd_cal_result_data;

}gsm_tx_cal_result_per_path_type;

typedef struct
{
   uint8 num_rfm_paths;

   gsm_tx_cal_result_per_path_type gsm_tx_cal_path_results[8];

} seq_cal_gsm_tx_res_type;
...
typedef struct
{
   ...
   seq_cal_gsm_tx_res_type gsm_tx_cal_data;
...
typedef enum
{
RX_RSB_CAL_ERROR_PRE_CAL_RSB_HIGH = 0,
RX_RSB_CAL_ERROR_POST_CAL_RSB_HIGH = 1,
RX_RSB_CAL_INVALID_ERROR = 0xFFFF
}rx_rsb_cal_error_type;

typedef struct
{
   uint8 bwType;

   uint16 errorTypeMask;//Each error type is defined by rx_rsb_cal_error_type enum
   ...
   uint16 debugDataMask;//Bit 0: PreCalRSB Data. Bit 1: PostCalRSB Data

   int16 preCalRSBValues[MAX_NUM_LO_FREQ_OFFSETS_RX_RSB];

   int16 postCalRSBValues[MAX_NUM_LO_FREQ_OFFSETS_RX_RSB];
...
typedef struct
{
   ...
   uint8 subband;
   uint16 band;
   ftm_rx_rsb_cal_data rsbCalData;
...
typedef enum
{
   ...
   SEQ_CAL_RESULT_TYPE_TX  = 1, 
   SEQ_CAL_RESULT_TYPE_RXGAIN  = 2, 
   SEQ_CAL_RESULT_TYPE_FBRXGAIN = 4, 
   SEQ_CAL_RESULT_TYPE_XPT_DBG_INFO = 8, 
   SEQ_CAL_RESULT_TYPE_FD_RXRSB = 16, 
   SEQ_CAL_RESULT_TYPE_XPT_DELAY = 32, 
   SEQ_CAL_RESULT_TYPE_GSM_TX = 64 
} seq_cal_res_type; // Used as bitmask
...
typedef struct
{
   ...
   union
   {
	  ...
	  GSM_V4_FBRx_Hw_Payload fbrxHwPayload;
   } slotPayload;
} GSM_V4_Slot_Payload;
...
typedef enum
{
   ...
   /*! Selected Bias > Vdmax and interpolated Bias > Vmax for calculated powers */
...
typedef enum
{
   ...
   FTM_RF_TEST_RADIO_CFG_PROP_CONTINUOUS_MODE = 8, /*!< 8 : Rx Mode: Burst or Continuous */
...
typedef enum
{
   FTM_RF_TEST_WAIT_TRG_PROP_UNASSIGNED = 0, /*!< 0 : Unassigned property */

   FTM_RF_TEST_WAIT_TRG_PROP_SUBSCR_IDX = 1, /*!< 1 : Subscription Index */

   FTM_RF_TEST_WAIT_TRG_PROP_TECHNOLOGY = 2, /*!< 2 : Technology */

   FTM_RF_TEST_WAIT_TRG_PROP_TX_CARRIER = 3, /*!< 3 : Transmit Carrier Number */

   FTM_RF_TEST_WAIT_TRG_PROP_RX_CARRIER = 4, /*!< 4 : Receiver Carrier Number */

   FTM_RF_TEST_WAIT_TRG_PROP_RFM_DEVICE = 5, /*!< 5 : Primary RFM device */

   FTM_RF_TEST_WAIT_TRG_PROP_RX_WAIT_THRESHOLD = 6, /*!< 6 : Threshold in dBm10 for the trigger */

   FTM_RF_TEST_WAIT_TRG_PROP_RX_WAIT_DIRECTION = 7, /*!< 7 : 0-Positive, 1-Negative */

   FTM_RF_TEST_WAIT_TRG_PROP_RX_WAIT_TIMEOUT = 8,  /*!< 8 : Timeout */

   FTM_RF_TEST_WAIT_TRG_PROP_RX_WAIT_MEAS_AGC = 9, /*!< 9 : Measured Rx AGC */

   FTM_RF_TEST_WAIT_TRG_PROP_NUM  /*!< Max : Defines maximum number of properties */
...
typedef enum
{
   ...
   FTM_RX_MEASURE_PROP_EST_SENSITIVITY  = 10, /*!< 10 : Measure Sensitivity */

   FTM_RX_MEASURE_PROP_CTON  = 11, /*!< 11 : Measure CtoN */

   FTM_RX_MEASURE_PROP_PEAK_FREQ = 12, /*!< 12 : Measure Peak Frequency */

   FTM_RX_MEASURE_PROP_FETCH_IQ = 13, /*!< 13 : Fetch Raw IQ samples */

   FTM_RX_MEASURE_PROP_NUM_IQ_SAMPLES = 14, /*!< 14 : Number of IQ samples used for IQ measurement */

   FTM_RX_MEASURE_PROP_FORMAT_OF_IQ_SAMPLES = 15, /*!< 15 : Format of the IQ samples used for IQ measurement. 1 - 16SC (16 bit signed complex) */
...
typedef enum
{
  ...
  FTM_TX_MEASURE_PROP_MEAS_REPORT_DEBUG_DATA,    /*!< 8 : Request TX IQ data */

  FTM_TX_MEASURE_PROP_MEAS_PHASE_ERROR,    /*!< 9 : Request Phase Error*/
  ...
  FTM_TX_MEASURE_PROP_PHASE_ERROR,    /*!< 33 : Reserved for future use */

  FTM_TX_MEASURE_PROP_MEAS_TX_TOTAL, /*!< 34 :  Total power across all carriers measurement action*/

  FTM_TX_MEASURE_PROP_TOTAL_PWR,  /*!< 35 : Measured total power across all carriers in dBm10 */

  FTM_TX_MEASURE_PROP_DEBUG_DATA, /*!< 36  : debug data in blob property format */

  FTM_TX_MEASURE_PROP_MEAS_ORFS_MOD,  /*!< 37 : Reserved for future use */

  FTM_TX_MEASURE_PROP_MEAS_ORFS_SWI,  /*!< 38 : Reserved for future use */
  ...
  FTM_TX_MEASURE_MEAS_ORFS_MOD_400KHZ_POS,

  FTM_TX_MEASURE_MEAS_ORFS_MOD_400KHZ_NEG,

  FTM_TX_MEASURE_MEAS_ORFS_MOD_600KHZ_POS,

  FTM_TX_MEASURE_MEAS_ORFS_MOD_600KHZ_NEG,

  FTM_TX_MEASURE_MEAS_ORFS_MOD_1200KHZ_POS,

  FTM_TX_MEASURE_MEAS_ORFS_MOD_1200KHZ_NEG,

  FTM_TX_MEASURE_MEAS_ORFS_MOD_1800KHZ_POS,

  FTM_TX_MEASURE_MEAS_ORFS_MOD_1800KHZ_NEG,

  FTM_TX_MEASURE_MEAS_ORFS_SWI_100KHZ_POS,

  FTM_TX_MEASURE_MEAS_ORFS_SWI_100KHZ_NEG,

  FTM_TX_MEASURE_MEAS_ORFS_SWI_200KHZ_POS,

  FTM_TX_MEASURE_MEAS_ORFS_SWI_200KHZ_NEG,

  FTM_TX_MEASURE_MEAS_ORFS_SWI_250KHZ_POS,

  FTM_TX_MEASURE_MEAS_ORFS_SWI_250KHZ_NEG,

  FTM_TX_MEASURE_MEAS_ORFS_SWI_400KHZ_POS,

  FTM_TX_MEASURE_MEAS_ORFS_SWI_400KHZ_NEG,

  FTM_TX_MEASURE_MEAS_ORFS_SWI_600KHZ_POS,

  FTM_TX_MEASURE_MEAS_ORFS_SWI_600KHZ_NEG,

  FTM_TX_MEASURE_MEAS_ORFS_SWI_1800KHZ_POS,

  FTM_TX_MEASURE_MEAS_ORFS_SWI_1800KHZ_NEG
...
typedef struct
{
   ...
   uint8 version; /*!< Version of the response packet  */
...
typedef struct
{
   ...
   uint16 associatedreserved; /* Reserved */
   uint8 nvcontainer; /*!< NV Container Index */
   uint8 rfmPathType; /*!< Defines Type of RFM path ; 0 = PRx; 1 = DRx 2 = Tx */
   uint8 actionType; /*!< 0 - Characterization 1 - Calibration 2 - Offset Calibration  */
} RFPathInformationPacketVersion0;

typedef struct
{
   uint16 band; /*!< Band*/
   uint8 subBand; /*!< Sub band; default value is 0.*/
   uint32 signalPath; /*!< RF Front End Path */
   uint16 rfmDevice; /*!< WTR Path */
   uint16 antennaPath; /* Antenna path */
   uint16 reserved; /* Reserved for future */
   uint32 associatedRxSignalPath; /*!< Associated Signal path */
   uint16 associatedRxRFMDevice; /*!< Associated RFM Device */
   uint16 associatedRxAntennaPath;/* Antenna path */
   uint16 associatedRxreserved; /* Reserved for future */
   uint8 nvcontainer; /*!< NV Container Index */
   uint8 rfmPathType; /*!< Defines Type of RFM Path; 0 = PRx; 1 = DRx 2 = Tx */
   uint8 txrxcalaction; /*!< 1 = Calibration 0 = Characterization 2 = offset calibration */
   uint32 calibrationreferenceSignalPath; /*< Signal path that shares(Full or partially) the cal data with current sig path*/
   uint32 associatedTxSignalPath; /*!< Associated Signal path */
   uint16 associatedTxRFMDevice; /*!< Associated RFM Device */
   uint16 associatedTxAntennaPath;/*< Antenna path */
   uint16 associatedTxreserved; /*< Reserved for future */
   uint8 antennaConfigState; /*< Antenna Switch state */
   uint8 rxrsbcalaction; /*!< 1 = Calibration 0 = Characterization 2 = offset calibration */
   uint8 antennaNumber;/*< Logical number associated to physical antenna port */
} RFPathInformationPacketVersion1;
...
typedef struct
{
   ...
   union
   {
      ...
      RFPathInformationPacketVersion1 version1; /*!< Version 1  of response packet */
...
typedef enum
{
   ...
   FTM_RF_PATH_INFORMATION_RESPONSE_VERSION1,
...
#define MAX_NUM_RX_CHAINS 4

typedef struct rf_path_assignment
{
   ...
   unsigned char isPRxOnly;
   unsigned char isRxOnly;
   unsigned char isHORxD;
   int tx_dev;
   int tx_sigpath;
   int tx_antpath;
   int rx_dev[MAX_NUM_RX_CHAINS];
   int rx_sigpath[MAX_NUM_RX_CHAINS];
   int rx_antpath[MAX_NUM_RX_CHAINS];
   unsigned char freqAdj_apply;
   long freqAdj_ppb;
   long freqAdj_centerFrequency;
   unsigned char freqAdj_enableXO;
   long freqAdj_totalAdjust;
   unsigned char continuous_mode;
   rf_path_assignment()
   {
      for(int i = 0; i < MAX_NUM_RX_CHAINS; i++)
      {
         rx_dev[i]      = -1;
         rx_sigpath[i]  = -1;
         rx_antpath[i]  = -1;
...
#define FTM_XPT_TXAGC_OVERRIDE_V2_MAX_CARRIERS 3

typedef struct
{
   byte                        iCarrierIdx;
   dword                       iSigPath;
   FTM_PriPathDevType_Enum     iRfmDevice;
   word                        iAntSwitchPath;
   FTM_xptType_Enum            iXPT_type;
   FTM_OverrideActionType_enum iIQOverrideType;
   int32                       iIQGain;
   FTM_OverrideActionType_enum iTxFeGainOverrideType;
   int32                       iTxFeGain;
   FTM_OverrideActionType_enum iEnvScOverrideType;
   int32                       iEnvScale;
   int16                       iRGI;
   int16                       iPABias;
   int8                        iPAState;
   int16                       iPACurr;
   FTM_OverrideActionType_enum iDelayOverrideType;
   int32                       iDelay_us;
} FTM_XPT_TxAgc_Override_V2_Subpacket_V0;
...
typedef struct tx_prop_values
{
   ...
   unsigned char enable_multislot_control; //This is set to true if multiple slots need to be enabled . In continuous mode this is set to false .
   unsigned long num_slots; //Number of slots to be enabled
   ftm_rf_test_multi_slot_params multi_slot_params[8];
   tx_prop_values()
   {
      enable_multislot_control = 0;
   }
} tx_prop_values;

typedef struct rx_prop_values
{
   unsigned long ftm_rf_tech; /*!< Technology information 0 - CDMA , 1 - WCDMA, 2 - GSM, 3- LTE, 4 - TDSCDMA */
   unsigned long subscriber;
   ...
   double exp_rxagc;
   unsigned char meas_rxagc;
   unsigned char report_lna_gain_state;
   unsigned char meas_est_sens;
   unsigned char meas_peak_freq;
   unsigned char meas_cton;
   unsigned char fetch_iq;
   rx_prop_values()
   {
      meas_rxagc              = 1;
      report_lna_gain_state   = 1;
      meas_est_sens           = 0;
      meas_peak_freq          = 0;
      meas_cton               = 0;
      fetch_iq                = 0;
...
typedef struct rx_measurements
{
   short rx_agc;
   short rx_lna_state;
   ...
   rx_measurements()
   {
      rx_agc = -9999;
      rx_lna_state = 99;
      rx_peak_frequency = 0xFFFFFFFF;
      rx_cton = -9999;
      rx_estimated_sensitivity = -9999;
   }
} rx_measurements;

typedef struct wait_trigger_prop_values
{
   unsigned long subscriber;
   unsigned long ftm_rf_tech; /*!< Technology information 0 - CDMA , 1 - WCDMA, 2 - GSM, 10- LTE, 11 - TDSCDMA */
   unsigned long rx_carr;
   int rx_dev;
   double rx_wait_dbm_threshold;
   unsigned char rx_wait_dbm_direction;
   unsigned long rx_wait_dbm_timeout;
   wait_trigger_prop_values()
   {
      rx_wait_dbm_threshold = -60.0;
      rx_wait_dbm_direction = 0; //0 - Positive Direction, 1 - Negative direction
      rx_wait_dbm_timeout = 5000000;
   }
}wait_trigger_prop_values;

typedef struct wait_trigger_measurments
{
   short rx_wait_dbm_measured_agc;
   wait_trigger_measurments()
   {
      rx_wait_dbm_measured_agc = -9999;
   }
}wait_trigger_measurments;

typedef struct tx_meas_prop_values
{
   unsigned long ftm_rf_tech; /*!< Technology information 0 - CDMA , 1 - WCDMA, 2 - GSM, 3- LTE, 4 - TDSCDMA */
   unsigned long subscriber;
   unsigned long tx_carrier;
   unsigned long tx_device;
   unsigned char meas_pwr;
   unsigned char meas_evm;
   unsigned char meas_aclr;
   unsigned char meas_vswr;
   unsigned char req_iq;
   unsigned char meas_sfl;
   unsigned char meas_total_pwr;
...
typedef struct tx_measurements
{
   double        tx_pwr;
   double        total_tx_pwr;
   double        evm;
   ...
   unsigned char is_supported_tx_pwr;
   unsigned char is_supported_total_tx_pwr;
   unsigned char is_supported_evm;
   unsigned char is_supported_aclr;
   unsigned char is_supported_vswr;
   unsigned char is_supported_spec_flat;
   ftm_rf_test_blob_data* blob_data;
   tx_measurements()
   {
      tx_pwr            = -999.0;
      total_tx_pwr      = -999.0;
      evm               = -999.0;
      aclr_utra1_pos    = -999.0;
      aclr_utra1_neg    = -999.0;
      aclr_utra2_pos    = -999.0;
      aclr_utra2_neg    = -999.0;
      aclr_eutra_pos    = -999.0;
      aclr_eutra_neg    = -999.0;
      vswr_ratio        = -999.0;
      vswr_return_loss  = -999.0;
      vswr_s11_mag      = -999.0;
      vswr_s11_phase    = -999.0;
      vswr_fwd_pwr      = -999.0;
      vswr_rev_pwr      = -999.0;
      spec_flat_r1      = -999.0;
      spec_flat_r2      = -999.0;
      spec_flat_r12     = -999.0;
      spec_flat_r21     = -999.0;
      spec_flat_r1_ext  = -999.0;
      spec_flat_r2_ext  = -999.0;
      spec_flat_r12_ext = -999.0;
      spec_flat_r21_ext = -999.0;
      is_supported_tx_pwr        = 0;
      is_supported_total_tx_pwr  = 0;
      is_supported_evm           = 0;
      is_supported_aclr          = 0;
      is_supported_vswr          = 0;
      is_supported_spec_flat     = 0;
   }
} tx_measurements;
