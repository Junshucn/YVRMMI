/******************************************************************************
 * *$Header: //depot/HTE/QDART/QMSL6.1/QMSL_WWAN/inc/QMSL_FTM_RF.h#117 $
 * *$DateTime: 2017/10/10 06:47:11 $
 *
...
extern "C" {
   ...
   *******************************************************************************/
   ...
   /******************************************************************************/
   /**
   !! Document Reference TB

   This command overrides Tx freq to a DL freq. It takes FDD DLL channel as the argument. 

   \param hResourceContext = Resource context that was returned from the call to ConnectServer()
   \param dl_channel = valid FDD DL channel
   \param piFTM_Error_Code = FTM Error code returned by FTM command
   *  0  Success
   *  1  Error
   \return true if successful, false if fail.
   ...
   \warning
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_TX_FREQ_OVERRIDE(  HANDLE hResourceContext, unsigned int dl_channel, unsigned char * piFTM_Error_Code );
   ...
   *******************************************************************************/
   ...
   FTM Common API Commands, 80-VA888-1: FTM_XPT_TXAGC_OVERRIDE_V2

   FTM_XPT_TX_AGC_OVERRIDE_V2 - Configures TX AGC Override.

   \param  [in]      hResourceContext - Phone Handle
   \param  [in]      iVersion = Version number of the FTM API. Currently 0 is supported.
   \param  [in]      iNumOfConfig = Number of carriers. For LTE single carrier and other techs, this should be 1. For ULCA, this can be > 1.
   \param  [in]      pFTM_XPT_TxAgc_Override_V2_Subpacket = Pointer to the first element of array of FTM_XPT_TxAgc_Override_V2_Subpacket structs.
                                                            The size of the array is equal to iNumOfConfig. Currently for version 0, struct to be
                                                            used is FTM_XPT_TxAgc_Override_V2_Subpacket_V0 - this struct is defined in QLib_defines.h file.
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_XPT_TXAGC_OVERRIDE_V2( HANDLE hResourceContext,
                                                          ...
                                                          unsigned char iNumOfConfig,
                                                          void * FTM_XPT_TxAgc_Override_V2_Subpacket );
   ...
   *******************************************************************************/
   ...
   \param antennaNum          The antenna number for tuners to be programmed
   ...
   \param impedanceScenario   Impedance scenario to program
   \param apertureScenario    Aperture scenario to program
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_RF_ANT_TUNER_SET_TUNER_PROPERTIES( HANDLE hResourceContext,
                                                                      ...
                                                                      unsigned char antennaNum,
                                                                      unsigned char updateType,
                                                                      unsigned char tunecodeSize,
                                                                      unsigned char* pTunecodeBytes,
                                                                      unsigned char impedanceScenario,
                                                                      unsigned char apertureScenario);
   ...
   *******************************************************************************/
   ...
   \param numAntennas				As an input parameter, specifies the max length of pTunerStatus
									As an output parameter, the number of antenna ports reported
   \param pTunerStatus				Pointer to an array of ftm_rffe_tuner_status_data_type structures
									that will be filled with the status of all tuners per antenna port.
									The array must have at least numAntennas elements.
   \param tunecodeDataAnt0			Pointer to a byte array filled with the tunecode data for Ant0
   \param tunecodeMappingAnt0		Pointer to a byte array filled with the chip mapping of tunecode data for Ant0
   \param tunecodeDataAnt1			Pointer to a byte array filled with the tunecode data for Ant1
   \param tunecodeMappingAnt1		Pointer to a byte array filled with the chip mapping of tunecode data for Ant1
   \param tunecodeDataAnt2			Pointer to a byte array filled with the tunecode data for Ant2
   \param tunecodeMappingAnt2		Pointer to a byte array filled with the chip mapping of tunecode data for Ant2
   \param tunecodeDataAnt3			Pointer to a byte array filled with the tunecode data for Ant3
   \param tunecodeMappingAnt3		Pointer to a byte array filled with the chip mapping of tunecode data for Ant3
   ...
   \warning This command will fail if the numAntennas input value is less than number 
            of antenna ports reported.
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_RFFE_GET_TUNER_STATUS( HANDLE hResourceContext,
                                                                  unsigned short version, 
                                                                  unsigned char source, 
                                                                  unsigned char* numAntennas,
																  void* pTunerStatusAnt0,
																  void* pTunerCodeDataAnt0,
																  void* pTunerCodeMappingAnt0,
																  void* pTunerStatusAnt1,
																  void* pTunerCodeDataAnt1,
																  void* pTunerCodeMappingAnt1,
																  void* pTunerStatusAnt2,
																  void* pTunerCodeDataAnt2,
																  void* pTunerCodeMappingAnt2,
																  void* pTunerStatusAnt3,
																  void* pTunerCodeDataAnt3,
																  void* pTunerCodeMappingAnt3);

	/******************************************************************************/
   /**
   FTM Common API Commands (80-VA888-1) FTM_GET_S11_MAG

   This command triggers the S11 measurement capture and calculates the S11 magnitude.
   
   \param hResourceContext Resource context that was returned from the call to QLIB_ConnectServer()
   \param tech				Technology
   \param carrier_index	Carrier index
   \param status			Command status
								0 = Success
								1 = Measurement Failure/not completed
								2 = Not Allowed (For example, TXAGC below supported measurement range)
								3 = In progress, try again when the current measurement is finished
								4 = Invalid argument
								5 = Measurement completed, but with bad/invalid measurement data
	\param s11_mag			S11 magnitude in dB100

   \return true if successful, false if fail.

   \warning None.
   *******************************************************************************/
	QLIB_API unsigned char QLIB_FTM_GET_S11_MAG( HANDLE hResourceContext,
																unsigned char tech, 
																unsigned char carrier_index, 
   ...
   *******************************************************************************/
   ...
   Factory Test Mode Commands Document 80-P5227-1
   ...
   *******************************************************************************/
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_RF_TEST_CMD_WAIT_TRIGGER API is a blocking call while waiting for triggering event using the FTM-RF-TEST subsystem
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_RF_TEST_CMD_WAIT_TRIGGER(   HANDLE hResourceContext,
                                                               void* request_data,
                                                               void* respsonse_data,
                                                               unsigned long* error_code,
                                                               unsigned long* specific_err_code );
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_RF_TEST_CMD_TX_CONTROL API is used to send a tx control command
   to the phone using the FTM-RF-TEST subsystem
   ...
   *******************************************************************************/
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_RF_TEST_CMD_RX_MEASURE_AND_FETCH_IQ_DATA API is used to get the Rx power measurements from the phone using the FTM-RF-TEST subsystem  and access IQ data through delayed response.
   ...
   \param [out] num_blobs = Number of blobs in response
   ...
   *******************************************************************************/
   ...
   QLIB_API unsigned char QLIB_FTM_RF_TEST_CMD_RX_MEASURE_AND_FETCH_IQ_DATA(  HANDLE hResourceContext,
                                                                              unsigned long subscriber,
                                                                              unsigned long technology, 
                                                                              void* request_data,
                                                                              void* response_data,
   ...
   *******************************************************************************/
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_RF_TEST_CMD_TX_MEASURE_AND_FETCH_DEBUG_DATA API is used to report TX measurements like Power, EVM, ACLR, etc. computed by the UE and access Debug Blob data through delayed response.
   ...
   *******************************************************************************/
   ...
   QLIB_API unsigned char QLIB_FTM_RF_TEST_CMD_TX_MEASURE_AND_FETCH_DEBUG_DATA(  HANDLE hResourceContext,
                                                                                 void* request_data,
                                                                                 void* response_data,
                                                                                 unsigned char* num_blobs,
                                                                                 void* response_blob_data,
   ...
   *******************************************************************************/
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_QUERY_ACTIVE_CLIENT_ALLOCATION API is used to get the RF device and sig_path info from TRM
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_QUERY_ACTIVE_CLIENT_ALLOCATION(  HANDLE hResourceContext,
                                                                    void* request_data,
                                                                    void* respsonse_data,
                                                                    unsigned long* error_code,
                                                                    unsigned long* specific_err_code );
   ...
   *******************************************************************************/
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_TEST_DEBUG_AUTO_PIN API is used to overwrite the Pin Accum values

   \param [in] requestData = Pointer to Array of Request Properties

   \param [out] error_code = Ftm error code

   \param [out] specific_err_code = error code specific to the subcommand
   ...
   *******************************************************************************/
   ...
   QLIB_API unsigned char QLIB_FTM_TEST_DEBUG_AUTO_PIN(  HANDLE hResourceContext,
                                                         void* request_data,
                                                         unsigned long* error_code, unsigned long* specific_err_code );

   /********************************************************************************/
   /**
   QLIB_FTM_TEST_DEBUG_AUTO_DELAY API is used to overwrite the Delay Accum values
   ...
   *******************************************************************************/
   ...
   QLIB_API unsigned char QLIB_FTM_TEST_DEBUG_AUTO_DELAY(  HANDLE hResourceContext,
                                                           void* request_data,
                                                           unsigned long* error_code, unsigned long* specific_err_code );

   /********************************************************************************/
   /**
   QLIB_FTM_TEST_DEBUG_AGC_ALGORITHM_CONTROL API is used to control the agc algorithms enabled on the device
   ...
   *******************************************************************************/
   ...
   QLIB_API unsigned char QLIB_FTM_TEST_DEBUG_AGC_ALGORITHM_CONTROL(  HANDLE hResourceContext,
                                                                      void* request_data,
                                                                      unsigned long* error_code, unsigned long* specific_err_code );
   ...
   *******************************************************************************/
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_OVERRIDE_TX_MTPL_PWR API is used to override Tx MTPL Power
   to the phone.

   Factory Test Mode Commands Document 80-VA888-1

   \param [in] technology = technology

   \param [in] enable_MTPL_Override = enable_MTPL_Override

   \param [in] Overriden_MTPL_PWR = Overriden_MTPL_PWR
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_OVERRIDE_TX_MTPL_PWR(  HANDLE hResourceContext,
															 unsigned char technology, 
															 unsigned char enable_MTPL_Override, 
															 unsigned short Overriden_MTPL_PWR,
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_RF_TEST_CMD_MSIM_CONFIG(  HANDLE hResourceContext,
	...
	/********************************************************************************/
   /**
   QLIB_FTM_RF_TEST_CMD_IRAT_CONFIGURE API is used to send a IRATconfig command
   to the phone using the FTM-RF-TEST subsystem
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_RF_TEST_CMD_IRAT_CONFIG(  HANDLE hResourceContext,
                                                             void* request_data,
                                                             unsigned long* error_code,
                                                             unsigned long* specific_err_code );
   ...
   *******************************************************************************/
   ...
   QLIB_API unsigned char QLIB_FTM_SET_TRM_ANT_PORT_STATE( HANDLE hResourceContext, unsigned char switch_position, unsigned char* trm_result, unsigned char *error_code);
	...
	/********************************************************************************/
   /**
   QLIB_FTM_GET_TRM_ANT_PORT_STATE API is used to set the port position for Port A/Port B testing
   ...
   *******************************************************************************/
   ...
   QLIB_API unsigned char QLIB_FTM_GET_TRM_ANT_PORT_STATE( HANDLE hResourceContext, unsigned char* error_code, unsigned char* current_port_status,unsigned char* current_cal_status,unsigned char* pending_port_status,unsigned char* pending_cal_status);

   /******************************************************************************/
   /**

   QLIB_FTM_SET_TRM_SAWLESS_STATE API is used to set the sawless state for tdd tech testing

   \param sawless_state = 0: default, 1: sawless

   \param trm_result = Flag indicating whether sawless request was ACKED :   0 - FALSE;   1 - TRUE
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_SET_TRM_SAWLESS_STATE( HANDLE hResourceContext, unsigned char sawless_state, unsigned char* trm_result, unsigned char* error_code );
   ...
   *******************************************************************************/
   ...
   QLIB_FTM_ANTSW_DISPATCH API is used to dispatch a diag command to request ASDIV (0x1841) log pkt repost.

   \param command (CMD) = 1: Repost Antsw logs, 2: Set Repost Log Timer  (1 - 60 secs param), 3: Get Debug Mask, 4: Set Debug Mask   (param)

   *******************************************************************************/
  QLIB_API unsigned char  QLIB_FTM_ANTSW_DISPATCH( HANDLE hResourceContext, unsigned char action );
   ...
   *******************************************************************************/
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_RF_DEBUG_CMD_RADIO_CONFIGURE: Use FTM RF Debug framework to set up radio

   \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()

   \param [in] technology: 0-CDMA, 1-WCDMA, 2-LTE, 3-WCDMA..

   \param [in]   pointer to ftm_rf_debug_property_data.  This data structure holds the (property, value) pair to configure the radio
   ...
   *******************************************************************************/
	QLIB_API unsigned char QLIB_FTM_RF_DEBUG_CMD_RADIO_CONFIGURE(  HANDLE hResourceContext,
																					unsigned long technology, 
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_RF_DEBUG_CMD_TX_OVERRIDE: Use FTM RF Debug framework to do Tx Override

   \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()

   \param [in]   pointer to ftm_rf_debug_property_data.  This data structure holds the (property, value) pair to perform Tx Override
   ...
   *******************************************************************************/
    QLIB_API unsigned char QLIB_FTM_RF_DEBUG_CMD_TX_OVERRIDE(  HANDLE hResourceContext,
                                                                                    unsigned long technology, 
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_RF_DEBUG_CMD_TX_OVERRIDE: Use FTM RF Debug framework to do Rx Override

   \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()

   \param [in]   pointer to ftm_rf_debug_property_data.  This data structure holds the (property, value) pair to perform Rx Override
   ...
   *******************************************************************************/
    QLIB_API unsigned char QLIB_FTM_RF_DEBUG_CMD_RX_OVERRIDE(  HANDLE hResourceContext,
                                                                                    unsigned long technology, 
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_RF_DEBUG_CMD_TX_MEASURE: Use FTM RF Debug framework to do FBRX measurement

   \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()

   \param [in]   pointer to ftm_rf_debug_property_data.  This data structure holds the (property, value) pair to perform FBRX measurement

   \param [out]   pointer to ftm_rf_debug_property_data.  This data structure holds the (property, value) pair from FBRX measurement data

   \return true if successful, false if fail.

   *******************************************************************************/
    QLIB_API unsigned char QLIB_FTM_RF_DEBUG_CMD_TX_MEASURE(  HANDLE hResourceContext,
                                                                                    unsigned long technology, 
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
      ...
      /********************************************************************************/
   /**
   QLIB_FTM_RF_DEBUG_CMD_RX_MEASURE: Use FTM RF Debug framework to do Rx Measure
   ...
   *******************************************************************************/
    QLIB_API unsigned char QLIB_FTM_RF_DEBUG_CMD_RX_MEASURE(  HANDLE hResourceContext,
                                                                                    unsigned long technology, 
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_RF_DEBUG_CMD_TX_MEASURE_ADV: Use FTM RF Debug framework to do Tx Measure Advanced
   ...
   *******************************************************************************/
    QLIB_API unsigned char QLIB_FTM_RF_DEBUG_CMD_TX_MEASURE_ADV(  HANDLE hResourceContext,
                                                                                    unsigned long technology, 
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
   ...
   *******************************************************************************/
     QLIB_API unsigned char QLIB_FTM_RF_DEBUG_CMD_LOAD_UNITY_DPD(  HANDLE hResourceContext,
                                                                                    unsigned long technology, 
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
   ...
   *******************************************************************************/
      ...
      QLIB_API unsigned char QLIB_FTM_RF_DEBUG_CMD_VDPD_CAL(  HANDLE hResourceContext,
                                                                                    unsigned long technology, 
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
   ...
   *******************************************************************************/
      ...
      QLIB_API unsigned char QLIB_FTM_RF_DEBUG_CMD_VDPD_CONVERSION(  HANDLE hResourceContext,
                                                                                    unsigned long technology, 
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_RF_DEBUG_CMD_LOAD_DPD: Use FTM RF Debug framework to Load DPD
   ...
   *******************************************************************************/
      ...
      QLIB_API unsigned char QLIB_FTM_RF_DEBUG_CMD_LOAD_DPD(  HANDLE hResourceContext,
                                                                                    unsigned long technology, 
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
    ...
    /********************************************************************************/
   /**
   QLIB_FTM_RF_DEBUG_CMD_SET_DPD_DEBUG_MODE: Use FTM RF Debug framework to Set DPD Debug Mode

   \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()

   \param [in]   pointer to ftm_rf_debug_property_data.  This data structure holds the (property, value) pair to set DPD debug mode

   \param [out]   pointer to ftm_rf_debug_property_data.  This data structure holds the (property, value) pair to set DPD debug mode

   \return true if successful, false if fail.

   *******************************************************************************/

      QLIB_API unsigned char QLIB_FTM_RF_DEBUG_CMD_SET_DPD_DEBUG_MODE(  HANDLE hResourceContext,
                                                                                    unsigned long technology, 
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_RF_DEBUG_CMD_DISCOVERY: Use FTM RF Debug framework to fetch supported FTM RF Debug commands

   \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()

   \param [in]   pointer to ftm_rf_debug_property_data.  This data structure holds the (property, value) pair to fetch supported commands

   \param [out]   pointer to ftm_rf_debug_property_data.  This data structure holds the (property, value) pair to fetch supported commands

   \return true if successful, false if fail.

   *******************************************************************************/

      QLIB_API unsigned char QLIB_FTM_RF_DEBUG_CMD_DISCOVERY(  HANDLE hResourceContext,
                                                                                    unsigned long technology, 
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_QLINK_BLER_TESST: Run a BLER test on a given HW block index

   \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()

   \param [in]   pointer to ftm_qlink_property_data.  This data structure holds the (property, value) pair to perform BLER test

   \param [out]   pointer to ftm_qlink_debug_property_data.  This data structure holds the (property, value) pair from BLER test

   \param [in/out] pointer to hold the ftm error code data

   \return true if successful, false if fail.

   *******************************************************************************/

    QLIB_API unsigned char QLIB_FTM_QLINK_BLER_TEST(  HANDLE hResourceContext,
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
                                                                                    unsigned long* specific_err_code );
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_QLINK_PING_TEST: Run a ping test on a given HW block index

   \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()

   \param [in]   pointer to ftm_qlink_property_data.  This data structure holds the (property, value) pair to perform ping test

   \param [out]   pointer to ftm_qlink_property_data.  This data structure holds the (property, value) pair from ping test
   ...
   *******************************************************************************/
    ...
    QLIB_API unsigned char QLIB_FTM_QLINK_PING_TEST(  HANDLE hResourceContext,
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
                                                                                    unsigned long* specific_err_code );
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_QLINK_READ_SLAVE_ID: QLINK Read Slave ID a given HW block index

   \param hResourceContext = Resource context that was returned from the call to QLIB_ConnectServer()

   \param [in]   pointer to ftm_qlink_property_data.  This data structure holds the (property, value) pair to read SLAVE ID

   \param [in/out]   pointer to ftm_qlink_property_data.  This data structure holds the (property, value) pair from reading SLAVE ID
   ...
   *******************************************************************************/
    ...
    QLIB_API unsigned char QLIB_FTM_QLINK_READ_SLAVE_ID(  HANDLE hResourceContext,
                                                                                    void* request_data,
                                                                                    void* respsonse_data,
                                                                                    unsigned long* error_code,
                                                                                    unsigned long* specific_err_code );
   ...
   /********************************************************************************/
   /**
   QLIB_FTM_QLINK_SHMOO_PLOT_TEST: Run a eye plot test on QLINK
   ...
   \param [in/out] pointer to size of the user provided buffer in pBuffer.  The actual size used is updated upon function return

   \param [in/out] pointer to user provided buffer to store the BLOB data

   \return true if successful, false if fail.

   *******************************************************************************/
    QLIB_API unsigned char QLIB_FTM_QLINK_SHMOO_PLOT_TEST(  HANDLE hResourceContext,
                                                  void* request_data,
                                                  void* response_data,
                                                  unsigned long* error_code,
                                                  unsigned long* specific_err_code,
                                                  unsigned long *piSizeOfBuffer,
                                                  unsigned long *pBuffer);


      /********************************************************************************/
   /**
   QLIB_FTM_QLINK_SHMOO_PLOT_TEST_V1: Run a eye plot test on QLINK
   ...
   *******************************************************************************/
    QLIB_API unsigned char QLIB_FTM_QLINK_SHMOO_PLOT_TEST_V1(  HANDLE hResourceContext,
                                                  void* request_data,
                                                  void* response_data,
                                                  unsigned long* error_code,
                                                  unsigned long* specific_err_code,
   ...
   /******************************************************************************/
   /**
   QLIB_FTM_UNIFIED_TX_INFO_QUERY - queries the DUT for Maximum Tx power capability
   ...
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_UNIFIED_TX_INFO_QUERY(  HANDLE hResourceContext,
                                                void* request_data,
                                                void* response_data,
                                                unsigned long* error_code,
                                                unsigned long* specific_err_code );
  ...
  /********************************************************************************/
   /**
   QLIB_FTM_UNIFIED_CA_CAPABILITY_QUERY -  For a given technology, this API obtains supported CA capability information that RF provides to network in order to indicate the level of support for CA combos
   ...
   \warning
   *******************************************************************************/
   QLIB_API unsigned char QLIB_FTM_UNIFIED_CA_CAPABILITY_QUERY( HANDLE hResourceContext,
                                                void* request_data,
                                                void* response_data,
                                                unsigned long* error_code,
                                                unsigned long* specific_err_code );
